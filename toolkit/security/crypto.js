...
module.exports = {
    ...
    encrypt: (input, encoding) => {
        ...
        let cipher = crypto.createCipheriv('aes-256-cbc', global.app.config.crypto.key, global.app.config.crypto.iv);
        return cipher.update(input, 'utf8', encoding) + cipher.final(encoding);
    },
    decrypt: (input, encoding) => {
        encoding = encoding || global.app.config.crypto.encoding;
        let decipher = crypto.createDecipheriv('aes-256-cbc', global.app.config.crypto.key, global.app.config.crypto.iv);
